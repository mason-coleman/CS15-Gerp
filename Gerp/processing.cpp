...
void traverseHelper(DirNode *node, std::string path)
{
    ...
    for (int i = 0; i < node->numSubDirs(); i++) {
        ...
        traverseHelper(subDir, path + "/" + subDir->getName());
    }
}
...
void traverseDirectory(string directory) {
    //Constructs FSTree rooted at given directory
    FSTree tree(directory);
    //Get the root so we can traverse
    DirNode *root = tree.getRoot();
    //call our recursive helper
    traverseHelper(root, root->getName());
}